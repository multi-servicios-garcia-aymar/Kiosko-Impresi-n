...
import { X, Upload, Check, Loader2, Megaphone, Clock, MapPin, AlignLeft, MousePointerClick, Users, Layout, Zap, Hash, Plus, Trash2, Film, Image as ImageIcon, Volume2, VolumeX, Info, FileText } from 'lucide-react';
  ...
  return (
    ...
    >
      <motion.div
        ...
        className="bg-white w-full max-w-2xl rounded-[2.5rem] shadow-2xl overflow-hidden flex flex-col max-h-[95dvh] relative"
      >
        <AnimatePresence>
          {showSpecsInfo && (
            <motion.div
              ...
              exit={{ opacity: 0, x: 20 }}
              className="absolute inset-0 z-[110] bg-white p-8 overflow-y-auto"
            >
              <div className="flex items-center justify-between mb-8">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="font-bold text-slate-900">Guía de Formatos</h3>
                    <p className="text-[10px] text-slate-400 uppercase font-bold tracking-widest">Especificaciones Técnicas</p>
                  </div>
                </div>
                <button 
                  onClick={() => setShowSpecsInfo(false)}
              ...
              <div className="grid gap-6">
                {[
                  {
                    title: 'Carrusel Principal (HD)',
                    aspect: '3:1 o 2:1 (Horizontal)',
                    size: '1920x640px o 1200x600px',
                    pos: 'Parte superior de la pantalla principal',
                    icon: <Layout className="w-4 h-4" />
                  },
                  {
                    title: 'Barra Lateral (Eco)',
                    aspect: '3:4 (Vertical)',
                    size: '600x800px',
                    pos: 'Columna derecha del kiosko',
                    icon: <AlignLeft className="w-4 h-4" />
                  },
                  {
                    title: 'Superpuesto (Max)',
                    aspect: 'Full Screen / 4:3',
                    size: '1080x1440px',
                    pos: 'Capa flotante principal',
                    icon: <Zap className="w-4 h-4" />
                  }
                ].map((spec, i) => (
                  <div key={i} className="p-6 bg-slate-50 rounded-[2rem] border border-slate-100 flex gap-4 items-start">
                    <div className="w-12 h-12 bg-white rounded-2xl shadow-sm flex items-center justify-center text-indigo-500 shrink-0 border border-slate-100">
                      {spec.icon}
                    </div>
                    <div>
                      <h4 className="font-bold text-slate-800 mb-1">{spec.title}</h4>
                      <div className="grid grid-cols-2 gap-x-4 gap-y-1">
                        <div>
                          <span className="text-[9px] uppercase font-bold text-slate-400 block">Relación Aspecto</span>
                          <span className="text-xs font-semibold text-slate-600">{spec.aspect}</span>
                        </div>
                        <div>
                          <span className="text-[9px] uppercase font-bold text-slate-400 block">Tamaño Sugerido</span>
                          <span className="text-xs font-semibold text-slate-600">{spec.size}</span>
                        </div>
                      </div>
                      <div className="mt-3 flex items-center gap-2 bg-indigo-500/5 px-3 py-2 rounded-xl">
                        <MapPin className="w-3 h-3 text-indigo-400" />
                        <span className="text-[10px] text-indigo-600 font-medium">{spec.pos}</span>
                      </div>
                    </div>
                  </div>
                ))}
              ...
              <div className="mt-8 p-6 bg-emerald-50 rounded-[2rem] border border-emerald-100">
                <h4 className="font-bold text-emerald-800 text-xs uppercase tracking-wider mb-2 flex items-center gap-2">
                  <FileText className="w-4 h-4" /> Formatos Soportados
                </h4>
                <p className="text-xs text-emerald-700 leading-relaxed font-medium">
                  • <b>Imágenes:</b> JPG, PNG, WEBP (Max 5MB)<br/>
                  • <b>Videos:</b> MP4, WEBM (H.264 recomendado, Max 20MB)<br/>
                  • <b>Duración:</b> Se recomienda 5-15 segundos por item.
                </p>
              </div>

              <button
                onClick={() => setShowSpecsInfo(false)}
                className="w-full mt-8 py-4 bg-slate-900 text-white rounded-[1.5rem] font-bold text-sm shadow-xl shadow-slate-200 hover:bg-slate-800 transition-all transition-all active:scale-95"
              >
                Entendido, Continuar
              </button>
            </motion.div>
